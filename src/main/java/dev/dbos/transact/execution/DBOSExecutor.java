...
public class DBOSExecutor {
  ...
  public void registerWorkflow(
      String workflowName, Object target, String targetClassName, Method method) {
  ...
  public WorkflowInitResult preInvokeWorkflow(
      String workflowName, String className, Object[] inputs, String workflowId, String queueName) {
    ...
    if (ctx.hasParent()) {
      systemDatabase.recordChildWorkflow(
          ctx.getParentWorkflowId(), ctx.getWorkflowId(), ctx.getParentFunctionId(), workflowName);
    }
   ...
   * @throws Throwable
   */
  <T> T runAndSaveResult(Object target, Object[] args, WorkflowFunction function, String workflowId)
      throws Throwable {
  ...
  public <T> WorkflowHandle<T> submitWorkflow(
    ...
    WorkflowInitResult initResult =
        preInvokeWorkflow(workflowName, targetClassName, args, wfId, null);
    ...
    ContextAwareCallable<T> contextAwareTask =
        new ContextAwareCallable<>(DBOSContextHolder.get().copy(), task);
  ...
  public <T> T runStep(
    ...
    if (workflowId == null) {
      throw new DBOSException(
          UNEXPECTED.getCode(), "No workflow id. Step must be called from workflow");
    }
      ...
      try {
        ...
        Throwable actual =
            (e instanceof InvocationTargetException)
                ...
                : e;
        logger.info(
            "After: step threw exception " + actual.getMessage() + "-----" + actual.toString());
        eThrown = actual;
      }
  ...
  public void submit(Runnable task) {
    ...
    ContextAwareRunnable contextAwareTask =
        new ContextAwareRunnable(DBOSContextHolder.get().copy(), task);
