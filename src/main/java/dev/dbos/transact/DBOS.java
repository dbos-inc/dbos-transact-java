...
public class DBOS {
    ...
    /**
     * Initializes the singleton instance of DBOS with config. Should be called once during app startup. @DBOSConfig
     * config dbos configuration
     */
    ...
    public static synchronized DBOS initialize(DBOSConfig config, SystemDatabase sd, DBOSExecutor de, QueueService q,
            SchedulerService ss) {
        if (instance != null) {
            throw new IllegalStateException("DBOS has already been initialized.");
        }
    ...
    public static class WorkflowBuilder<T> {
        ...
        public T build() {
            ...
            if (async) {
                ...
                return QueueInvocationHandler.createProxy(interfaceClass,implementation,queue,
                        DBOS.getInstance().dbosExecutor);
            } else {
                return UnifiedInvocationHandler.createProxy(interfaceClass,implementation,
                        DBOS.getInstance().dbosExecutor);
            }
        }
    }
    ...
    /**
     * Scans the class for all methods that have Workflow and Scheduled annotations and schedules them for execution
     *
    ...
    /**
     * Durable sleep. When you are in a workflow, use this instead of Thread.sleep. On restart or during recovery the
     * original expected wakeup time is honoured as opposed to sleeping all over again.
     *
     * @param seconds
     *            in seconds
     */
    ...
    /***
     *
     * Cancel the workflow. After this function is called, the next step (not the current one) will not execute
     *
     * @param workflowId
     */
    ...
    /**
     * Fork the workflow. Re-execute with another Id from the step provided. Steps prior to the provided step are copied
     * over
     *
     * @param workflowId
     ...
     * @param options
     *            {@link ForkOptions} containing forkedWorkflowId, applicationVersion, timeout
    ...
    /**
     * Start a workflow asynchronously. If a queue is specified with DBOSOptions, the workflow is queued.
     *
     * @param func
