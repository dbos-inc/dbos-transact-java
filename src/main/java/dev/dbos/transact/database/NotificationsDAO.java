...
public class NotificationsDAO {
    ...
    public void send(String workflowUuid, int functionId, String destinationUuid, Object message, String topic)
            throws SQLException {
        ...
        try (Connection conn = dataSource.getConnection()) {
            ...
            try {
                ...
                if (recordedOutput != null) {
                    logger.debug(String.format("Replaying send, id: %d, destination_uuid: %s, topic: %s",functionId,
                            destinationUuid,finalTopic));
                    conn.commit();
                    return;
                } else {
                    logger.debug(String.format("Running send, id: %d, destination_uuid: %s, topic: %s",functionId,
                            destinationUuid,finalTopic));
                }
    ...
    public Object recv(String workflowUuid, int functionId, int timeoutFunctionId, String topic, double timeoutSeconds)
        ...
        try {
            ...
            if (!success) {
                // This should not happen, but if it does, it means the workflow is executed
                // concurrently
        ...
        try (Connection conn = dataSource.getConnection()) {
            ...
            try {
                ...
                String deleteAndReturnSql = " WITH oldest_entry AS ( "
                        + " SELECT destination_uuid, topic, message, created_at_epoch_ms " + " FROM %s.notifications "
                        + " WHERE destination_uuid = ? AND topic = ? " + " ORDER BY created_at_epoch_ms ASC "
                        + " LIMIT 1 " + " ) " + " DELETE FROM %s.notifications "
    ...
    public void setEvent(String workflowId, int functionId, String key, Object message) throws SQLException {
        ...
        try (Connection conn = dataSource.getConnection()) {
            ...
            try {
                // Check if operation was already executed
                StepResult recordedOutput = stepsDAO.checkStepExecutionTxn(workflowId,functionId,functionName,conn);
                ...
                String upsertSql = " INSERT INTO %s.workflow_events (workflow_uuid, key, value) " + " VALUES (?, ?, ?) "
    ...
    public Object getEvent(String targetUuid, String key, double timeoutSeconds, GetWorkflowEventContext callerCtx)
            throws SQLException {
        ...
        if (callerCtx != null) {
            ...
            try (Connection conn = dataSource.getConnection()) {
                recordedOutput = stepsDAO.checkStepExecutionTxn(callerCtx.getWorkflowId(),callerCtx.getFunctionId(),
                        functionName,conn);
            }
        ...
        try {
            ...
            try (Connection conn = dataSource.getConnection(); PreparedStatement stmt = conn.prepareStatement(getSql)) {
            ...
            if (value == null) {
                ...
                if (callerCtx != null) {
                    ...
                    actualTimeout = stepsDAO.sleep(callerCtx.getWorkflowId(),callerCtx.getTimeoutFunctionId(),
                            timeoutSeconds,true // skip_sleep
                    );
                }
