...
public class QueuesDAO {
    ...
    public List<String> getAndStartQueuedWorkflows(Queue queue, String executorId, String appVersion)
        ...
        try (Connection connection = dataSource.getConnection()) {
            ...
            if (queue.hasLimiter()) {
                String limiterQuery = "SELECT COUNT(*) " + " FROM %s.workflow_status " + " WHERE queue_name = ? "
                        + " AND status != 'ENQUEUED' " + " AND started_at_epoch_ms > ? ;";
            ...
            if (queue.getWorkerConcurrency() > 0 || queue.getConcurrency() > 0) {
                ...
                String pendingQuery = "SELECT executor_id, COUNT(*) as task_count " + " FROM %s.workflow_status "
                        + " WHERE queue_name = ? " + " AND status = 'PENDING' " + " GROUP BY executor_id; ";
                ...
                if (queue.getWorkerConcurrency() > 0) {
                    if (localPendingWorkflows > queue.getWorkerConcurrency()) {
                        logger.warn(String.format(
                                ...
                                localPendingWorkflows,queue.getName(),queue.getWorkerConcurrency()));
                    }
                ...
                if (queue.getConcurrency() > 0) {
                    int globalPendingWorkflows = pendingWorkflowsDict.values().stream().mapToInt(Integer::intValue)
                            .sum();
                    ...
                    if (globalPendingWorkflows > queue.getConcurrency()) {
                        logger.warn(String.format(
                                ...
                                globalPendingWorkflows,queue.getName(),queue.getConcurrency()));
                    }
            ...
            queryBuilder.append(" SELECT workflow_uuid " + " FROM %s.workflow_status " + " WHERE queue_name = ? "
                    + " AND status = 'ENQUEUED' " + " AND (application_version = ? OR application_version IS NULL) ");
            ...
            if (!dequeuedIds.isEmpty()) {
                logger.trace(String.format("[%s] dequeueing %d task(s)",queue.getName(),dequeuedIds.size()));
            }
            ...
            String updateQuery = "UPDATE %s.workflow_status " + " SET status = 'PENDING', "
                    + " application_version = ?, " + " executor_id = ?, " + " started_at_epoch_ms = ?, "
                    + "workflow_deadline_epoch_ms = CASE "
                    + "    WHEN workflow_timeout_ms IS NOT NULL AND workflow_deadline_epoch_ms IS NULL "
                    + "       THEN ? + workflow_timeout_ms " + "     ELSE workflow_deadline_epoch_ms " + "      END "
                    + " WHERE workflow_uuid = ?;";
