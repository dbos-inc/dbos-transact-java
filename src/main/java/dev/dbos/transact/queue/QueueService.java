...
public class QueueService {
          ...
          try {
            ...
            List<String> workflowIds =
                systemDatabase.getAndStartQueuedWorkflows(
                    queue, Constants.DEFAULT_EXECUTORID, Constants.DEFAULT_APP_VERSION);
  ...
  public synchronized boolean isStopped() {
    // If the workerThread reference is null, it implies it hasn't started or has been fully cleaned
    // up.
    if (workerThread == null) {
      return true;
    }
    // The most definitive check: if the latch has counted down to zero, the worker's run() method
    // has completed.
