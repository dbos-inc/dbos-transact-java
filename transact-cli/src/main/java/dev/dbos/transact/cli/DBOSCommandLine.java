...
@Command(name = "dbos", mixinStandardHelpOptions = true, subcommands = {
    InitCommand.class,
...
public class DBOSCommandLine implements Runnable, IVersionProvider {
  ...
  public static class DatabaseOptions {
    @Option(names = { "-D",
        "--db-url" }, description = "Your DBOS system database URL (defaults to DBOS_SYSTEM_JDBC_URL env var)")
    String url;

    @Option(names = { "-U",
        "--db-user" }, description = "user name for your DBOS system database (defaults to PGUSER env var)")
    String user;

    @Option(names = { "-P",
        "--db-password" }, description = "password for your DBOS system database (defaults to PGPASSWORD env var)", arity = "0..1", interactive = true)
    String password;

    public DBOSClient createClient() {
      var url = Objects.requireNonNullElseGet(
          this.url, () -> System.getenv(Constants.SYSTEM_JDBC_URL_ENV_VAR));
      var user = Objects.requireNonNullElseGet(
          this.user, () -> System.getenv(Constants.POSTGRES_USER_ENV_VAR));
      var password = Objects.requireNonNullElseGet(
...
class MigrateCommand implements Runnable {
  ...
  @Mixin
  DBOSCommandLine.DatabaseOptions dbOptions;
...
class ResetCommand implements Callable<Integer> {
  ...
  public Integer call() throws Exception {
    ...
    if (!skipConfirmation) {
      String prompt = "This command resets your DBOS system database, deleting metadata about past workflows and steps. Are you sure you want to proceed?";
    ...
    try (var conn = DriverManager.getConnection(pair.url(), dbOptions.user, dbOptions.password);
      ...
      DBOSCommandLine.logger.info("System database has been reset successfully {}", pair.database());
      return 0;
...
@Command(name = "worfklow", subcommands = {
...
class WorfklowListCommand implements Runnable {
  ...
  @Option(names = { "-s",
      "--start-time" }, description = "Retrieve workflows starting after this timestamp (ISO 8601 format)")
  String startTime;

  @Option(names = { "-e",
      "--end-time" }, description = "Retrieve workflows starting befor this timestamp (ISO 8601 format)")
  String endTime;

  @Option(names = { "-S",
      "--status" }, description = "Retrieve workflows with this status (PENDING, SUCCESS, ERROR, ENQUEUED, CANCELLED, or MAX_RECOVERY_ATTEMPTS_EXCEEDED)")
  String status;

  @Option(names = { "-n", "--name" }, description = "Retrieve workflows with this name")
  String name;

  @Option(names = { "-v", "--app-version" }, description = "Retrieve workflows with this application version")
  String appVersion;

  @Option(names = { "-q", "--queue" }, description = "Retrieve workflows on this queue")
  String queue;
  ...
  @Option(names = { "-d", "--sort-desc" }, description = "Sort the results in descending order (older first)")
  boolean sortDescending;

  @Option(names = { "-l", "--limit" }, description = "Limit the results returned", defaultValue = "10")
  int limit;

  @Option(names = { "-o", "--offset" }, description = "Offset for pagination", defaultValue = "0")
  int offset;

  @Option(names = { "-Q", "--queues-only" }, description = "Retrieve only queued workflows")
  boolean queuesOnly;
...
class WorfklowStepsCommand implements Runnable {
  ...
  public void run() {
    var client = dbOptions.createClient();
    var steps = client.listWorkflowSteps(
        Objects.requireNonNull(workflowId, "workflowId parameter cannot be null"));
...
@Command(name = "cancel", description = "Cancel a workflow so it is no longer automatically retried or restarted")
...
class WorfklowResumeCommand implements Runnable {
  ...
  public void run() {
    var client = dbOptions.createClient();
    var handle = client.resumeWorkflow(
        Objects.requireNonNull(workflowId, "workflowId parameter cannot be null"));
...
class WorfklowForkCommand implements Runnable {
  ...
  @Option(names = { "-f", "--forked-workflow-id" }, description = "Custom workflow ID for the forked workflow")
  String forkedWorkflowId;

  @Option(names = { "-v", "--application-version" }, description = "Application version for the forked workflow")
  String appVersion;

  @Option(names = { "-s", "--step" }, description = "Restart from this step [default: 1]", defaultValue = "1")
  Integer step;
